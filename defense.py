...
class Defense:
    ...
    def defend(self, request: dict) -> dict:
        ...
        if total_power > 1e-12:
            high_ratio = np.sum(power[freqs > 2500.0]) / total_power
        else:
            high_ratio = 0.0

        # 特征3：频谱平坦度
        power_p = power + 1e-12
        sfm = np.exp(np.mean(np.log(power_p))) / (np.mean(power_p) + 1e-12)
        sfm = np.clip(sfm, 0.0, 1.0)

        # ---- 唯一变化：w_sfm 10.0 → 12.0 ----
        score = 16.0 * high_ratio + 30.0 * zcr + 12.0 * sfm - 4.0
        prob = 1.0 / (1.0 + np.exp(-score))
        return {"spoof_probability": float(np.clip(prob, 0.0, 1.0))}
