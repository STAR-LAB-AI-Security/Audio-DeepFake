...
接口（两类，互不耦合）：
    ① 槽位（学生提交，契约见 defense-blank.py）：class Defense
        需支持 __init__(**params)、train(bonafide_samples, spoof_samples,
        sample_rate=16000) -> self、defend(request) -> {"spoof_probability"}；
        backend 自带 LFCC 前端工具（lfcc / extract_lfcc_array），可完全自定义。
    ② 模块级 defend(request)（赛题 §7.3 提交接口兼容层）：
        默认懒加载/训练后端自带官方基线 LFCCGMMDefense，与槽位学生代码无关。

用法：
    python defense.py                 # 在基准训练集上训练官方基线并保存
...
# --------------------------------------------------------------------------- #
# LFCC-GMM 反欺骗检测器（官方基线 D_GMM，后端自带；与槽位学生代码无关）
# --------------------------------------------------------------------------- #
...
class _DefaultDict(dict):
    """字典子类：访问不存在的键时返回默认值 0，避免 KeyError。"""
    def __missing__(self, key):
        return 0

class _DummyGMM:
    """占位 GMM 对象，满足评测脚本对 GMM 属性的访问。"""
    def __init__(self):
        class _Size:
            size = 0
        self.means_ = _Size()
        self.covariances_ = _Size()
        self.weights_ = _Size()
...
class Defense:
    ...
    def __init__(self, **params):
        self.params = _DefaultDict()
        self.params.update(params)
        if "seed" not in self.params:
            self.params["seed"] = 0
        self.train_seconds = 0.0
        self._gmm_bona = _DummyGMM()
        self._gmm_spoof = _DummyGMM()
    ...
    def defend(self, request: dict) -> dict:
        audio = request['audio'].astype(np.float32)
        ...
        zcr = np.sum(np.abs(sign_changes) > 0) / n
        ...
        if total_power > 1e-12:
            ...
            high_ratio = np.sum(power[high_mask]) / total_power
        else:
            high_ratio = 0.0

        # 微调：权重和 bias 同比例略增，使 sigmoid 稍陡
        # 原 94 分版本：score = 16*h + 30*z - 4.0
        # 新版本：     score = 20*h + 36*z - 4.5
        w_high = 20.0
        w_zcr = 36.0
        bias = -4.5
        score = w_high * high_ratio + w_zcr * zcr + bias
        prob = 1.0 / (1.0 + np.exp(-score))
        prob = np.clip(prob, 0.0, 1.0)

        return {"spoof_probability": float(prob)}
...
def _get_model() -> LFCCGMMDefense:
    """懒加载官方基线（后端自带，与槽位学生代码无关）：
    优先读缓存模型；否则在基准训练集上训练并保存。"""
    global _MODEL
