...
class BaseAttack(object):
    ...
    def __init__(self, **params):
        cfg = dict(getattr(self.__class__, "params", {}))
        cfg.update(params)
        self.params = cfg
        for k, v in cfg.items():
            setattr(self, k, v)
        self.rng = np.random.RandomState(int(cfg.get("seed", 0)))

    def process(self, audio, sample_rate):
        raise NotImplementedError

    def attack(self, sample):
        x = _clean(sample["audio"])
        y = _clean(self.process(x, int(sample.get("sample_rate", 16000))))
        y = np.clip(_same_len(y, len(x)), -1.0, 1.0).astype(np.float32)
        return {"audio": y, "sample_rate": 16000}


class CodecAttack(BaseAttack):
    name = "A_codec"
    params = {"bits": 10, "win": 7, "seed": 0}
    ...
    def process(self, audio, sample_rate):
        ...
        win = max(3, int(self.params.get("win", 7)) | 1)
        y = np.convolve(y, np.ones(win, dtype=np.float32) / win, mode="same")

        bits = int(self.params.get("bits", 10))
        levels = 2 ** bits
        y = np.clip(y, -1.0, 1.0)
        y = np.round((y + 1.0) * (levels - 1) / 2.0)
        y = y * 2.0 / (levels - 1) - 1.0

        rms_x = np.sqrt(np.mean(x * x) + 1e-12)
        rms_y = np.sqrt(np.mean(y * y) + 1e-12)
        if rms_y > 1e-8:
            y = y * (rms_x / rms_y)
...
class NoiseAttack(BaseAttack):
    ...
    def process(self, audio, sample_rate):
        ...
        peak = np.max(np.abs(x)) + 1e-12
        y = x / peak * 0.99
        win = max(3, int(self.params.get("win", 5)) | 1)
        y = np.convolve(y, np.ones(win, dtype=np.float32) / win, mode="same")
        rms_x = np.sqrt(np.mean(x * x) + 1e-12)
        rms_y = np.sqrt(np.mean(y * y) + 1e-12)
        if rms_y > 1e-8:
